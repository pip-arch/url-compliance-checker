#!/usr/bin/env python3
"""
Resume Admiral Markets processing with extended skip list.
Adds guitarsxs.com to avoid more timeouts.
"""
import subprocess
import sys
import os
...
# Extended problematic domains to skip
SKIP_DOMAINS = {
    ...
    'guitarsxs.com',  # Connection timeouts (NEW)
}

print("🔄 RESUMING Admiral Markets Processing - 50 Workers (v2)")
print("=" * 60)
print(f"📊 Resuming at: {datetime.now()}")
print(f"📈 Current blacklist: 1,322 entries (+238 since last run)")
print(f"🔍 Already found: 391 URLs with Admiral mentions")
print(f"⚙️  Workers: 50")
print(f"🚫 Skipping domains: {', '.join(SKIP_DOMAINS)}")
print()
print("✨ NEW: Added guitarsxs.com to skip list")
print("🐛 FIX: Addressing Crawl4AI parameter issue")
print()

# Create a filtered URL file
print("Step 1: Creating filtered URL list...")
filtered_file = "data/tmp/admiral_filtered_resume_v2.csv"
...
log_file = f"data/logs/admiral_resume_50workers_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
print("ℹ️  Progress will be shown every 10 URLs processed")
print()
...
try:
    with open(log_file, 'w') as log:
        ...
        for line in iter(process.stdout.readline, ''):
            ...
            if urls_processed > 0 and urls_processed % 10 == 0:
                print(f"\n📊 Progress: {urls_processed} URLs checked, {admiral_found} with Admiral mentions, {blacklisted} blacklisted\n")
