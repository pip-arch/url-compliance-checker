#!/usr/bin/env python3
"""
Script to reanalyze URLs with multiple fallback options:
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("data/reanalysis_multi_fallback.log"),
...
GPT4_COST_PER_1K = 0.01  # $0.01 per 1K tokens
...
async def analyze_with_openai(url_content):
    """
    Analyze URL content using OpenAI API directly.
    
    Args:
        url_content: The URL content object to analyze
        
    Returns:
        Analysis result object or None if analysis fails
    """
    ...
    try:
        # Extract text from content in the same way blacklist_keywords does
        ...
        Focus on identifying issues related to financial regulation, misleading claims, scams, phishing, or other problematic content.
        """
        
        user_prompt = f"""URL: {url}
        
        Content:
        {content_text[:8000]}  # Limit content length to prevent token limits
...
async def reanalyze_with_multi_fallback(skip_first=1000, max_additional=500):
    """
    Reanalyze URLs with multiple fallback options: OpenRouter -> OpenAI -> Keywords
    ...
    logger.info(f"Using multi-fallback approach: OpenRouter ({current_model}) -> OpenAI (gpt-4-turbo) -> Keywords")
    ...
    try:
        ...
        batch_id = f"reanalysis_multi_fallback_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        for i in range(0, len(results_to_process), batch_size):
            ...
            for result in batch:
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            logger.info(f"Falling back to OpenAI analysis for URL: {url}")
                            analysis_result = await analyze_with_openai(url_content)
                            analysis_method = "openai"
                            batch_openai += 1
                            total_openai += 1
                            logger.info(f"Successfully analyzed URL {url} using OpenAI")
                            
                            # Calculate OpenAI cost
                            estimated_tokens = AVG_TOKENS_PER_ANALYSIS
                            cost_openai += (estimated_tokens / 1000) * GPT4_COST_PER_1K
                        except Exception as e_openai:
                            logger.warning(f"OpenAI analysis failed for URL {url}: {str(e_openai)}")
                            
                            # Use keyword fallback as final option
                    ...
                    if analysis_result and analysis_result.category == URLCategory.BLACKLIST:
                        logger.info(f"Blacklisted URL: {url} (using {analysis_method})")
                        batch_blacklisted += 1
                        total_blacklisted += 1
            ...
            logger.info(f"Batch methods: {batch_openrouter} OpenRouter, {batch_openai} OpenAI, {batch_keyword} keyword")
            ...
            # Calculate percentages for used methods
            total_methods = total_openrouter + total_openai + total_keyword
            if total_methods > 0:
                openrouter_pct = total_openrouter / total_methods * 100
                openai_pct = total_openai / total_methods * 100
                keyword_pct = total_keyword / total_methods * 100
                logger.info(f"Analysis methods: {total_openrouter} OpenRouter ({openrouter_pct:.1f}%), {total_openai} OpenAI ({openai_pct:.1f}%), {total_keyword} keyword ({keyword_pct:.1f}%)")
            
            logger.info(f"Speed: {urls_per_second:.2f} URLs/sec, Est. remaining: {est_remaining_time/60:.1f} min")
            logger.info(f"Cost: ${(cost_llama + cost_openai):.4f} (OpenRouter: ${cost_llama:.4f}, OpenAI: ${cost_openai:.4f})")
            ...
            if new_blacklisted_urls and (i + batch_size >= len(results_to_process) or len(new_blacklisted_urls) >= 10):
                try:
                    field_names = ["URL", "Main Domain", "Reason", "Confidence", "Category", "Compliance Issues", "Analysis Method", "Batch ID", "Timestamp"]
        ...
        total_cost = cost_llama + cost_openai
        
        logger.info(f"Reanalysis complete: {total_processed} URLs analyzed, {total_blacklisted} newly blacklisted")
        ...
        logger.info(f"Estimated cost: ${total_cost:.4f} (OpenRouter: ${cost_llama:.4f}, OpenAI: ${cost_openai:.4f})")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Reanalyze URLs with multiple fallback options")
    ...
    # Run the reanalysis
    asyncio.run(reanalyze_with_multi_fallback(args.skip_first, args.max_additional)) 