...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("data/reanalysis_remaining.log"),
...
async def reanalyze_remaining_urls(skip_first=1000, max_additional=500):
    """
    ...
    try:
        ...
        batch_id = f"reanalysis_remaining_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        total_cost_claude = (total_processed * AVG_TOKENS_PER_ANALYSIS / 1000) * CLAUDE_3_OPUS_COST_PER_1K
        total_cost_llama = (total_processed * AVG_TOKENS_PER_ANALYSIS / 1000) * LLAMA_70B_COST_PER_1K
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Reanalyze remaining URLs stored in Pinecone")
    ...
    # Run the reanalysis
    asyncio.run(reanalyze_remaining_urls(args.skip_first, args.max_additional)) 